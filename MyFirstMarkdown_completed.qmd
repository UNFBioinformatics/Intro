...
```{r question4}
# sparkles should have 10 rows: 6 Premium + 4 Ideal
...
# Yes, sparkles has the expected 10 rows.
```
...
```{r question9}
# The datasets contain only a small number of diamonds, so the sample may not
# represent the overall relationship.
# In addition, the plots do not control for other variables that can affect
# price, such as carat, color, etc.
```
...
```{r question10}
# I used the entire diamonds dataset instead of only 21 rows and compared
# price distributions across all five cut categories. This gives many more
# observations for each category and makes the comparison stronger.
ggplot(data = diamonds, aes(x = cut, y = price)) +
  geom_boxplot()

# I was surprised that the price distributions overlap so much.
# This shows that cut alone does not determine a diamond's price, and other
# variables such as carat also matter.
```
