...
```{r question4}
# sparkles should have 10 rows.

premium <- diatask[grep("Premium", diatask$cut), ]
ideal <- diatask[grep("Ideal", diatask$cut), ]

sparkles <- rbind(premium, ideal)

print(sparkles, n = nrow(sparkles))

# Yes, I got the expected 10 rows.
```
...
```{r question9}
# The plots use only 21 consecutive rows from the diamonds dataset,
# so the sample is too small and may not be representative.
# Other variables also affect price, so these plots are not enough
# to make a strong conclusion about cut, carat, and price.
```
...
# I used all rows from the diamonds dataset and selected the carat
# and price variables. This gives a much larger dataset than diatask.
# The plot shows that larger-carat diamonds are generally associated
# with higher prices. I was surprised by the amount of price variation
# among diamonds with similar carat values.
