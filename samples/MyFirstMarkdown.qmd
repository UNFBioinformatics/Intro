...
```{r question4}
#there should be 10 rows in 'sparkles'
print(diatask)
premium  <- diatask[diatask$cut== "Premium", ]
ideal <- diatask[diatask$cut== "Ideal", ]
sparkles <- rbind(premium, ideal)
print(sparkles)
#yes I got 10 for both
```
...
```{r question5}
q5 <-ggplot(data = diatask, aes(x = cut, y = price, color = cut)) +
  geom_point(alpha = 0.8, size =4)
ggsave("q5.png", plot = q5)
```
...
```{r question6}
q6 <-ggplot(data = diatask, aes(x = cut, y = price, color = cut)) +
  geom_point(alpha = 0.8, size =4) + theme(panel.grid = element_blank())
ggsave("q6.png", plot = q6)
...
```{r question8}
q8 <-ggplot(data = diasize, aes(x = carat, y = price, color = cut)) +
  geom_point(alpha = 0.8, size =4) + theme(panel.grid = element_blank())
ggsave("q8.png", plot = q8)
```
...
```{r question9}
## The plots do not account for other factors, especially carat, which strongly affects price. Therefore, they cannot show whether price differences are caused by cut or diamond size.
```
...
```{r question10}
my_data <- diamonds[diamonds[, 3] == "E", c(1, 2, 3, 7)]
my_data <- head(my_data, 1000)
q10 <-ggplot(data = my_data, aes(x = carat, y = price, color = cut)) +
  geom_point(alpha = 0.8, size =4) + theme(panel.grid = element_blank())
ggsave("q10.png", plot = q10)
head(diamonds)
```
