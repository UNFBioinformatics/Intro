...
```{r question4}
# Part A: Looking at diatask, there are 6 Premium and 4 Ideal rows, so sparkles should have 10 rows total.

# Part B: Filter for Premium and Ideal cuts using grep and combine them with rbind()
premium_df <- diatask[grep("Premium", diatask$cut),]
ideal_df   <- diatask[grep("Ideal", diatask$cut),]

sparkles <- rbind(premium_df, ideal_df)
sparkles

# Part C: Yes, sparkles has 10 rows, which matches the expected count from Part A.
```
...
```{r question5}
ggplot(data = diatask, mapping = aes(x = cut, y = price)) + geom_point()
```
...
```{r question6}
ggplot(data = diatask, mapping = aes(x = cut, y = price)) +
...
```{r question8}
ggplot(data = diasize, mapping = aes(x = carat, y = price)) +
  geom_point()
```
...
```{r question9}
## The plots for diatask and diasize are insufficient because:
# 1. Very Small Sample Size: They only use 21 rows (rows 80 to 100) out of the 50,000+ diamonds, which is not a representative sample of the overall dataset.
# 2. Confounding Variables: Looking at cut or carat in isolation ignores other key variables that affect price (such as clarity, color, and carat weight combined).
# 3. Limited Range: Rows 80-100 only capture a tiny subset of price and carat ranges, leading to potential sampling bias.
```
...
```{r question10}
# Constructing the dataset using all rows in diamonds to get a full, representative sample
...
# Reflection Comments:
# - What I did to construct the dataset: Instead of slicing a tiny subset of rows (like 80:100), 
#   I used the full 'diamonds' dataset so all carat sizes and price ranges were fully represented.
# - Were you surprised by the results?: It was interesting to see how steeply price increases as carat size increases, showing a strong positive relationship with higher variability in price for larger diamonds!
