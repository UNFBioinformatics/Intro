...
library(ggplot2)
ggplot(data = diatask, aes(x = cut, y = price, color = cut)) + geom_point(size = 4)
...
ggplot(data = diatask, aes(x = cut, y = price, color = cut)) + geom_point(size = 4) + theme_classic()
...
diasize <- diamonds[80:100, c("carat", "price", "cut")]
...
ggplot(data = diasize, aes(x = carat, y = price, color =cut)) + geom_point(size = 4)
...
Sample Size is what you need to consider. Not the other variables that wont be driving the changes.
Subsetting only 21 rows doesnt provide enough.
data points are needed to have clear statistical trend
...
big_data <- diamonds[1:1000,]
ggplot(data = big_data, aes(x = carat, y = price , color = cut)) + geom_point()


I constructed a larger data set using 1000 rows getting accurate distribution 
NOt surprised to see strong positive exponential relationship
carat diamonds cost more
