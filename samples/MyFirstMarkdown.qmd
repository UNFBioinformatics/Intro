...
```{r question4}
# Show all rows so I can count Premium + Ideal
print(diatask, n = 21)

# Expected number of rows in sparkles:
# 7
...
```{r question9}
# The plots are insufficient because diatask and diasize only contain 21 rows.
# This is too small to show real trends. The full diamonds dataset has 53,940 rows,
# so using only 21 points cannot reveal true relationships between cut, carat, and price.
```
...
```{r question10}
ideal_big <- diamonds[diamonds$cut == "Ideal", c("carat", "price")]
ggplot(data = ideal_big, aes(x = carat, y = price)) +
...
# I used all Ideal diamonds from the full dataset so I could see a real trend.
# With thousands of points instead of 21, the relationship between carat and price becomes clear: price increases as carat increases. I was not surprised by the result.
