...
```{r question1}
head(diatask)
#Cut and price are key variables
...
Sparkles=10 rows
print(diatask,n=row(diatask))

premium_df <- diatask[grep("Premuim", diatask$cut), ]
ideal_df <- [grep("Ideal", diatask$cut), ]
sparkles  <- rbind(premium_rows , ideal_rows)

sparkles
nrow(sparkles)

It has 10 rows, as predicted. 
'''
...
```{r question9}
# Individual plots only look at one variable at a time and dataset is too small to make any large conclusions. 
# Variable bias is possible based on diamond size, cut, carat weight, and clarity
```
