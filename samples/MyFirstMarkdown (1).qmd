...
```{r question1}
head (diatask)
#The key variables in diatask are cut and price.
```
...
```{r question4}
# I expect sparkles to have 10 rows.
...
# Yes, sparkles has 10 rows, which matches my prediction.
```
...
# I removed the grey gridlines by adding theme_classic().
```
...
```{r question9}
# The diatask and diasize plots only use 21 rows from the full diamonds
# dataset. This is too small a subset to make a strong conclusion.
# The rows were also selected by position, not randomly, so they may not
# represent the full dataset. Price may also be related to other variables,
# such as color, clarity, cut, and carat.
```
...
# I used all rows from the diamonds dataset and selected the carat and
# price variables. Using the full dataset gives much stronger evidence
# than using only 21 rows.
#
# I found that diamonds with larger carat values generally have higher
# prices. I was not surprised by the overall relationship, but I was
# surprised that diamonds with similar carat values can still have very
# different prices. This suggests that other diamond characteristics also
# affect price.
