...
#it says the matches are in rows 2 and 4 
...
```{r question4}
#10 
premium_dias <- diatask[grep ("Premium" , diatask$cut), ]
ideal_dias <- diatask[grep ("Ideal" , diatask$cut), ]
sparkles <- rbind(premium_dias, ideal_dias)
print(sparkles)
#i suppose 
```
...
ggplot(data = diatask, aes(x = cut, y = price, color = cut)) +
  geom_point(alpha = 0.6, size =1)+theme(panel.grid = element_blank()) 
```
...
ggplot(data = diasize, aes(x = carat, y = price, color = cut)) +
  geom_point(alpha = 0.1, size =4)+theme(panel.grid = element_blank()) 
```
...
```{r question10}
diaty <- diamonds[, c(1,7)] 

ggplot(data = diaty, aes(x = carat, y = price, color = carat)) +
  geom_point(alpha = 0.1, size = 0.2)+theme(panel.grid = element_blank()) 
```
