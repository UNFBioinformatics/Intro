...
```{r question4}
table(diatask$cut)
# there should be 10 rows for sparkles because there is 6 premium rows and 4 ideal rows
...
#Yes, I got the same number of rows I expected.
```
...
```{r question6}
ggplot(data = diatask, aes(x = cut, y = price, color = cut)) +
  geom_point(alpha = 0.8, size =4) + theme_classic()
...
```{r question8}
# recreate diasize here to ensure the chunk has the data it needs
diasize <- diamonds[80:100, c("carat", "cut", "price")]
...
```{r question9}
## The plots show a small bit of data, this small amount of data is not enough to make a good conclusion.
```
...
```{r question10}
 carat_data <- diamonds[, c("carat", "price")]
ggplot(data = carat_data, aes(x = carat, y = price)) +
  geom_point(alpha = 0.8, size =4) + theme_classic()

# I decided to use the full diamonds data set because it has A wide variety of options, this makes the trend more accurate. It wasn’t really surprising That price tends to increase as carat increases.
